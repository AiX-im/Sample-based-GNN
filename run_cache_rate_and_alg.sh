...
algs=("gcn" "graphsage") #"graphsage" "gcn" "gat"
models=("GCNSAMPLEPDCACHE" "GCNSAMPLECACHE" "GSSAMPLEPDCACHE" "GSSAMPLECACHE")
# echo ${#batch_sizes[@]}
...
do
    ...
    do
        ...
        do
            for(( l=0;l<${#models[@]}/2;l++))
            do
                old_file="${algs[i]}_${datasets[j]}_sample.cfg"
                new_file="${algs[i]}_${datasets[j]}_${cache_rates[k]}_${models[i*2+l]}_sample.cfg"
                new_type="${datasets[j]}_${cache_rates[k]}_${models[i*2+l]}"
                cat ${old_file} > ${new_file}
                sed -i '$d' ${new_file}
                sed -i '1d' ${new_file}
                echo "ALGORITHM:${models[i*2+l]}" >> ${new_file}
