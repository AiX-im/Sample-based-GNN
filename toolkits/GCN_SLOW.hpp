...
public:
 ...
 //GraphOperation *gt;
  ...
  GCN_SLOW_impl(Graph<Empty> *graph_, int iterations_, bool process_local = false,
  ...
  /*GPU dist*/ void run() {
    if (graph->partition_id == 0)
      printf("GNNmini::Engine[Dist.GPU.GCNimpl] running [%d] Epochs\n",
             iterations);
    // graph->print_info();
      std::random_device d;    // random_device下面会介绍
      std::mt19937 m(d());  //初始化生成器
      // 创建一个随机数分布，比如生成在 [1, 100] 范围内的整数
      std::uniform_int_distribution<int> dist(1, 100);
      int start_sleep_epoch = 0;  // 开始睡眠的epoch
      const int sleep_duration = 5; // 默认连续睡眠3个epoch
      bool is_sleeping = false;     // 如果是在睡眠的话就不用进行随机数生成判断
      long sleep_time = 0;
      auto sleep = [&](bool isSleep, long sleep_time){
          if(!isSleep){
              return ;
          }
          cudaDeviceSynchronize();
          std::printf("进程%d epoch %d sleep %ld (ms)\n", graph->partition_id, graph->rtminfo->epoch, sleep_time);
          std::printf("进程%d 睡眠前时间戳: %.4lf\n",graph->partition_id, get_time());
          std::this_thread::sleep_for(std::chrono::milliseconds(sleep_time));
          std::printf("进程%d 睡眠后时间戳: %.4lf\n",graph->partition_id, get_time());
      ...
      exec_time -= get_time();
      long epoch_time = 0; // 毫秒为单位的计时
      double run_time = -get_time();
    for (int i_i = 0; i_i < iterations; i_i++) {
        if(i_i - start_sleep_epoch >= sleep_duration) {
            is_sleeping = false;
        }
        if(!is_sleeping && i_i != 0) {
            if(dist(m) <= 50){
                is_sleeping = true;
                sleep_time = epoch_time*5;
                start_sleep_epoch = i_i;
            }

        }
      graph->rtminfo->epoch = i_i;
        epoch_time = -std::chrono::duration_cast<std::chrono::milliseconds>(
      ...
      Update();
        ...
        epoch_time += std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::system_clock::now().time_since_epoch()).count();
      if (graph->partition_id == 0)
        std::cout << "GNNmini::Running.Epoch[" << i_i << "]:loss\t" << loss.item<float>()
                  << "\ttime: " << epoch_time << std::endl;
    }
    run_time += get_time();
