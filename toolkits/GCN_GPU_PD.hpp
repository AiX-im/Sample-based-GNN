...
public:
  ...
  GCN_GPU_PD_impl(Graph<Empty> *graph_, int iterations_,
  ...
  void Forward(FastSampler* sampler, int type=0) {
      ...
      while(sampler->sample_not_finished()){
           ...
           sampler->load_embedding_gpu(Y_i , gnndatum->dev_local_embedding);
           ...
           //ctx->PushdownGraphOp<nts::op::SingleGPUSampleGraphOp>(sg,graph,1,Y_i,X[0]);
  ...
  void run() {
    ...
    nts::op::init_embedding(Y_PD, gnndatum->local_embedding,graph);
