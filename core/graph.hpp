...
public:
 ...
 // std::vector<COOChunk *> graph_shard_in;
  ...
  std::string filename; // edge_file文件的路径

  int partition_id;     // 本partition的id
  int partitions;       // partition的数量
  ...
  size_t unit_size;
  size_t edge_unit_size;    // 在无边数据的情况下，为两个顶点的大小，4*2字节
  int message_unit_size;

  bool symmetric;       // 是否是无向图
  VertexId vertices; // 节点的数量
  EdgeId edges;      // 边的数量
  EdgeId local_edges;
  VertexId *out_degree;         // VertexId [vertices]; numa-aware, out_degree of local vertices
  VertexId *in_degree;          // VertexId [vertices]; numa-aware, in degree of local vertices
  VertexId *in_degree_backward; //_new_0

  VertexId *out_degree_for_backward; // VertexId [vertices]; numa-aware, out_degree of all vertices
  VertexId *in_degree_for_backward;  // VertexId [vertices]; numa-aware, in degree of all vertices

  VertexId *partition_offset;       // VertexId [partitions+1], each partition start index (end index)
  VertexId *local_partition_offset; // VertexId [sockets+1], partition的numa再划分

  VertexId owned_vertices;          // local partition vertex num
  ...
  AdjUnit<EdgeData> **
      incoming_adj_list; // AdjUnit<EdgeData> [sockets] [vertices+1]; numa-aware
...
//  }
  ...
  void init_message_buffer() {
    output_cpu_buffer = (ValueType *)cudaMallocPinned(
        ((long)vertices) * gnnctx->max_layer * sizeof(ValueType));
    if (output_cpu_buffer == NULL)
      printf("allocate fail\n");
  }
#endif
  void init_communicatior() {
    NtsComm->init(partition_offset, owned_vertices, partitions, sockets,
  ...
  void init_gnnctx(std::string layer_string) {
    ...
    while (std::getline(ss, number, '-')) {
      ...
      gnnctx->max_layer =
          std::max(gnnctx->max_layer, (size_t)std::stoi(number));
      // printf("layers %d\n", std::stoi(number));
    }
...
//    }
  ...
  void init() {
    ...
    for (int t_i = 0; t_i < threads; t_i++) {
      thread_state[t_i] = (ThreadState *)numa_alloc_onnode(sizeof(ThreadState),
                                                           get_socket_id(t_i));
...
#pragma omp parallel for
    for (VertexId v_i = partition_offset[partition_id];
         v_i < partition_offset[partition_id + 1]; v_i++) {
      array[v_i] = value;
    }
  }
...
#pragma omp parallel for
  ...
  template <typename T> T *alloc_vertex_array(int size) { // create a
    char *array =
        (char *)mmap(NULL, size * sizeof(T) * vertices, PROT_READ | PROT_WRITE,
                     MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    assert(array != NULL);
    for (int s_i = 0; s_i < sockets; s_i++) {
      numa_tonode_memory(
          array + size * sizeof(T) * local_partition_offset[s_i],
          size * sizeof(T) *
  ...
  template <typename T> T *alloc_vertex_array_local(int size) { // create a
    ...
    for (int s_i = 0; s_i < sockets; s_i++) {
      numa_tonode_memory(
          array + size * sizeof(T) *
                      (local_partition_offset[s_i] - local_partition_offset[0]),
          size * sizeof(T) *
              (local_partition_offset[s_i + 1] - local_partition_offset[s_i]),
          s_i);
    }
  ...
  template <typename T> T *alloc_vertex_array() { // create a
    char *array =
        (char *)mmap(NULL, sizeof(T) * vertices, PROT_READ | PROT_WRITE,
                     MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    assert(array != NULL);
    for (int s_i = 0; s_i < sockets; s_i++) {
      numa_tonode_memory(array + sizeof(T) * local_partition_offset[s_i],
                         sizeof(T) * (local_partition_offset[s_i + 1] -
  ...
  template <typename T> T *alloc_vertex_array_local() { // create a
    char *array = (char *)mmap(NULL,
                               sizeof(T) * (partition_offset[partition_id + 1] -
                                            partition_offset[partition_id]),
                               PROT_READ | PROT_WRITE,
                               MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    assert(array != NULL);
  ...
  template <typename T> T *alloc_pointer_vertex_array() { // create a
    char *array =
        (char *)mmap(NULL, sizeof(T) * vertices, PROT_READ | PROT_WRITE,
                     MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    assert(array != NULL);
  ...
  template <typename T> T *dealloc_vertex_array(T *array) {
  ...
  template <typename T> void gather_vertex_array(T *array, int root) {
    if (partition_id != root) {
      MPI_Send(array + partition_offset[partition_id],
               sizeof(T) * owned_vertices, MPI_CHAR, root, GatherVertexArray,
               MPI_COMM_WORLD);
    } else {
      for (int i = 0; i < partitions; i++) {
        if (i == partition_id)
  ...
  long get_max_partition_size() {
    ...
    for (int i = 0; i < partitions; i++) {
      for (int s_i = 0; s_i < sockets; s_i++) {
        max_partition_size =
            std::max(max_partition_size,
                     (long)(partition_offset[i + 1] - partition_offset[i]));
      }
    }
  ...
  int get_local_partition_id(VertexId v_i) {
    for (int s_i = 0; s_i < sockets; s_i++) {
      if (v_i >= local_partition_offset[s_i] &&
  ...
  void load_undirected_from_directed(std::string path, VertexId vertices) {
    ...
    for (int i = 0; i < partitions; i++) {
      ...
      if (remained_partitions == 1) {
        ...
        partition_offset[i + 1] = (partition_offset[i + 1]) / PAGESIZE *
    ...
    MPI_Allreduce(partition_offset, global_partition_offset, partitions + 1,
                  vid_t, MPI_MAX, MPI_COMM_WORLD);
    for (int i = 0; i <= partitions; i++) {
      assert(partition_offset[i] == global_partition_offset[i]);
    }
    MPI_Allreduce(partition_offset, global_partition_offset, partitions + 1,
                  vid_t, MPI_MIN, MPI_COMM_WORLD);
    for (int i = 0; i <= partitions; i++) {
      assert(partition_offset[i] == global_partition_offset[i]);
    }
#ifdef PRINT_DEBUG_MESSAGES
    if (partition_id == 0) {
      for (int i = 0; i < partitions; i++) {
        ...
        printf("|V'_%d| = %u |E_%d| = %lu\n", i,
               partition_offset[i + 1] - partition_offset[i], i,
               part_out_edges);
      }
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    delete[] global_partition_offset;
    {
      ...
      for (int s_i = 0; s_i < sockets; s_i++) {
        ...
        for (VertexId v_i = local_partition_offset[s_i];
             v_i < local_partition_offset[s_i + 1]; v_i++) {
          remained_amount -= out_degree[v_i] + alpha;
        ...
        printf("|V'_%d_%d| = %u |E_%d| = %lu\n", partition_id, s_i,
               local_partition_offset[s_i + 1] - local_partition_offset[s_i],
               partition_id, sub_part_out_edges);
#endif
      }
    }
    ...
    for (int s_i = 0; s_i < sockets; s_i++) {
      ...
      outgoing_adj_index[s_i] =
          (EdgeId *)numa_alloc_onnode(sizeof(EdgeId) * (vertices + 1), s_i);
    }
    {
      std::thread recv_thread_dst([&]() {
        ...
        while (finished_count < partitions) {
          ...
          assert(recv_status.MPI_TAG == ShuffleGraph && i >= 0 &&
                 i < partitions);
          int recv_bytes;
          ...
          if (recv_bytes == 1) {
            finished_count += 1;
            char c;
            MPI_Recv(&c, 1, MPI_CHAR, i, ShuffleGraph, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
            continue;
          }
          assert(recv_bytes % edge_unit_size == 0);
          int recv_edges = recv_bytes / edge_unit_size;
          MPI_Recv(recv_buffer, edge_unit_size * recv_edges, MPI_CHAR, i,
                   ShuffleGraph, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
          // #pragma omp parallel for
      ...
      while (read_bytes < bytes_to_read) {
        ...
        for (EdgeId e_i = 0; e_i < curr_read_edges; e_i++) {
          ...
          memcpy(send_buffer[i].data() + edge_unit_size * buffered_edges[i],
                 &read_edge_buffer[e_i], edge_unit_size);
          buffered_edges[i] += 1;
    ...
    for (int s_i = 0; s_i < sockets; s_i++) {
      ...
      outgoing_adj_list[s_i] = (AdjUnit<EdgeData> *)numa_alloc_onnode(
          unit_size * outgoing_edges[s_i], s_i);
    }
    {
      std::thread recv_thread_dst([&]() {
        int finished_count = 0;
...
#pragma omp parallel for
  ...
  void transpose() {
    ...
    std::swap(compressed_outgoing_adj_vertices,
    ...
    void load_directed(std::string path, VertexId vertices) {
        double load_directed_time =-get_time();
        double prep_time = 0;
        prep_time -= MPI_Wtime();
        filename = path;
        symmetric = false;
      ...
      printf("|V| = %u, |E| = %lu\n", vertices, edges);
        ...
        read_bytes = 0;
        while (read_bytes <
               bytes_to_read) { // load the graph with a big chunk iteratively
            long curr_read_bytes;
            if (bytes_to_read - read_bytes > edge_unit_size * CHUNKSIZE) {
                curr_read_bytes =
                        read(fin, read_edge_buffer, edge_unit_size * CHUNKSIZE);
...
#pragma omp parallel for
        ...
        std::printf("first read time: %.4lf (s)\n", load_directed_time + get_time());
        //printf("partition_id:%d,out_degree[0]:%d,out_degree[1]:%d\n",partition_id,out_degree[0],out_degree[1]);
        MPI_Allreduce(MPI_IN_PLACE, out_degree, vertices, vid_t, MPI_SUM,
                      MPI_COMM_WORLD);
      ...
      for (int i = 0; i < partitions; i++) {
        ...
        delete[] global_partition_offset;
        {
            ...
            for (int s_i = 0; s_i < sockets; s_i++) {
               ...
               local_partition_offset[s_i + 1] - local_partition_offset[s_i],
               partition_id, s_i, sub_part_out_edges);
#endif
            }
        }
        // **************************************************************************Partition
...

        {
            ...
            MPI_Allreduce(MPI_IN_PLACE, in_degree_for_backward, vertices, vid_t,
...
#pragma omp parallel for
        ...
        for (int s_i = 0; s_i < sockets; s_i++) {
            ...
            for (VertexId p_v_i = 0; p_v_i < compressed_incoming_adj_vertices[s_i];
                 p_v_i++) {
            ...
            incoming_adj_list[s_i] = (AdjUnit<EdgeData> *)numa_alloc_onnode(
                    unit_size * incoming_edges[s_i], s_i);
        }
        {
            std::thread recv_thread_src([&]() {
                int finished_count = 0;
...
#pragma omp parallel for
        ...
        prep_time += MPI_Wtime();
        load_directed_time += get_time();
        std::printf("load directed time: %.4lf (s)\n", load_directed_time);
      ...
      for (int t_i = 0; t_i < threads; t_i++) {
        ...
        if (remained_partitions == 1) {
          ...
          for (VertexId p_v_i = tuned_chunks_dense[i][t_i].curr;
               p_v_i < tuned_chunks_dense[i][t_i].end; p_v_i++) {
            got_edges += compressed_incoming_adj_index[s_i][p_v_i + 1].index -
                         compressed_incoming_adj_index[s_i][p_v_i].index +
                         alpha;
          }
  ...
  R process_vertices(std::function<R(VertexId)> process, Bitmap *active) {
    ...
    for (int t_i = 0; t_i < threads; t_i++) {
      ...
      thread_state[t_i]->end = local_partition_offset[s_i] +
                               partition_size / threads_per_socket /
...
#pragma omp parallel reduction(+ : reducer)
  ...
  template <typename M> inline size_t sizeofM(int f_size) {
  ...
  void flush_local_send_buffer_buffer(int t_i, int f_size) {
    ...
    if (local_send_buffer[t_i]->count != 0)
      memcpy(send_buffer[current_send_part_id][s_i]->data +
                 (sizeofM<ValueType>(f_size)) * pos,
             local_send_buffer[t_i]->data,
             (sizeofM<ValueType>(f_size)) * local_send_buffer[t_i]->count);
    local_send_buffer[t_i]->count = 0;
  }

  void flush_data_to_send_buffer_buffer_lock_free_init(
      VertexId message_count_partition) {
    int pos = __sync_fetch_and_add(&send_buffer[current_send_part_id][0]->count,
                                   message_count_partition);
    // if (pos == 0)
    //  printf("send buffer is NULL(%d)\n", pos);
  }

  void flush_data_to_send_buffer_buffer_lock_free_write(
      int t_i, int f_size, VertexId key, ValueType *value,
    ...
    // local_send_buffer[t_i]->count);
    int pos =
        message_write_offset_key; // if (local_send_buffer[t_i]->count != 0)
    if (pos < 0 || pos >= (partition_offset[current_send_part_id + 1] -
    ...
    memcpy(send_buffer[current_send_part_id][s_i]->data +
           ...
           &key, sizeof(VertexId));
    memcpy(send_buffer[current_send_part_id][s_i]->data +
               (sizeofM<ValueType>(f_size)) * pos + sizeof(VertexId),
           value, f_size * sizeof(ValueType));
  }
  ...
  void emit_buffer(VertexId vtx, t_v *buffer, int f_size) {
    ...
    memcpy(s_buffer +
               local_send_buffer[t_i]->count * sizeofM<ValueType>(f_size),
           &vtx, sizeof(VertexId));
    memcpy(s_buffer +
               local_send_buffer[t_i]->count * sizeofM<ValueType>(f_size) +
               sizeof(VertexId),
           buffer, sizeof(ValueType) * f_size);
  ...
  R process_edges_forward_decoupled_dynamic_length(
      ...
      std::function<void(VertexId, CSC_segment_pinned *, char *,
                         std::vector<VertexId> &, VertexId)>
          sparse_slot,
...
#pragma omp parallel for
     ...
     */
    template <typename R, typename M>
    R batch_local_vertex_operation(
            std::function<void(VertexId, CSC_segment_pinned *, VertexId)> sparse_slot,
            std::vector<CSC_segment_pinned *> &graph_partitions, int feature_size,
            Bitmap *active, Bitmap *dense_selective = nullptr) {
        omp_set_num_threads(threads);
...
#pragma omp parallel for
  ...
  R process_edges_forward_decoupled_mutisockets(
      std::function<void(VertexId, int)> sparse_signal,
      std::function<void(VertexId, CSC_segment_pinned *, MessageBuffer **,
                         std::vector<VertexIndex> &, VertexId)>
          sparse_slot,
      std::vector<CSC_segment_pinned *> &graph_partitions, int feature_size,
      Bitmap *active, Bitmap *dense_selective = nullptr) {
    omp_set_num_threads(threads);
              ...
              dense_signal(
                  ...
                  VertexAdjList<EdgeData>(
                      incoming_adj_list_backward[s_i] +
                          compressed_incoming_adj_index_backward[s_i][p_v_i]
                              .index,
                      incoming_adj_list_backward[s_i] +
                          compressed_incoming_adj_index_backward[s_i][p_v_i + 1]
                              .index),
                  thread_id, i);
            }
          }
                    ...
                    VertexAdjList<EdgeData>(
                        incoming_adj_list_backward[s_i] +
                            compressed_incoming_adj_index_backward[s_i][p_v_i]
                                .index,
                        incoming_adj_list_backward[s_i] +
                            compressed_incoming_adj_index_backward[s_i]
                                                                  [p_v_i + 1]
                                                                      .index),
                    thread_id, i);
              }
      ...
      NtsVar &Y, int feature_size) // backward
  {
    ...
    size_t basic_chunk = 64;
    {
      ...
      if (process_overlap) {
        current_send_part_id = partition_id;
        Nts->InitBlockSimple(
            graph_partitions[(current_send_part_id + 1) % partitions], rtminfo,
            feature_size, feature_size, (current_send_part_id + 1) % partitions,
            layer_);
        Nts->ZeroVarMem(output_gpu_buffered, GPU_T);
        Nts->GatherBySrcFromDst(output_gpu_buffered, input_gpu_or_cpu,
                                input_gpu_or_cpu);
...
#pragma omp parallel
      ...
      for (int step = 0; step < partitions; step++) {
        ...
        for (int s_i = 0; s_i < sockets; s_i++) {
          Nts->DeviceSynchronize();
          Nts->AggMsgToMaster(Y, used_buffer[s_i]->data,
...
#pragma omp parallel for
  ...
  R forward_single_edge(NtsVar &message,
    ...
    for (int step = 0; step < partitions; step++) {
      int i = 0;
      Nts->InitBlock(graph_partitions[i], rtminfo, feature_size, feature_size,
                     current_recv_part_id, layer_);
  ...
  template <typename R, typename M>
  R backward_single_edge(
      NtsVar &input_grad, std::vector<CSC_segment_pinned *> &graph_partitions,
      ...
      int feature_size) // backward
  {
    ...
    cpp = 0;
    Nts->InitBlock(graph_partitions[current_send_part_id], rtminfo,
                   feature_size, feature_size, current_send_part_id, layer_);
...
//  }
  ...
  void generate_backward_structure() {
    ...
    compressed_incoming_adj_index_backward =
        new CompressedAdjIndexUnit *[sockets];
    ;
    ...
    for (int i = 0; i < sockets; i++) {
      ...
      incoming_adj_index_backward[i] =
          (EdgeId *)numa_alloc_onnode(sizeof(EdgeId) * (vertices + 1), i);
      memset(incoming_adj_index_backward[i], 0,
             sizeof(EdgeId) * (vertices + 1));
    }
    int local_edges = 0;
    //    printf("finish_1_%s\n",filename.c_str());
    EdgeUnit<EdgeData> *read_edge_buffer = new EdgeUnit<EdgeData>[CHUNKSIZE];
    ...
    while (read_bytes <
      ...
      for (EdgeId e_i = 0; e_i < curr_read_edges; e_i++) {
        ...
        for (int s_i = 0; s_i < sockets; s_i++) {
          if (local_partition_offset[s_i] <= dst &&
              dst < local_partition_offset[s_i + 1]) {
    ...
    for (int s_i = 0; s_i < sockets; s_i++) {
      ...
      // }
      incoming_adj_list_backward[s_i] = (AdjUnit<EdgeData> *)numa_alloc_onnode(
          unit_size * incoming_edges_backward[s_i], s_i);

      for (int i = 0; i < vertices; i++) {
        if (this->incoming_adj_bitmap_backward[s_i]->get_bit(i)) {
          compressed_incoming_adj_vertices_backward[s_i] += 1;
        }
        incoming_adj_index_backward[s_i][i + 1] +=
            incoming_adj_index_backward[s_i][i];
      }
      this->compressed_incoming_adj_index_backward[s_i] =
          (CompressedAdjIndexUnit *)numa_alloc_onnode(
              sizeof(CompressedAdjIndexUnit) *
                  (compressed_incoming_adj_vertices_backward[s_i] + 1),
              s_i);
      ...
      for (int i = 0; i < vertices; i++) {
        if (this->incoming_adj_bitmap_backward[s_i]->get_bit(i)) {
          ...
          compressed_incoming_adj_index_backward
              [s_i][compressed_incoming_adj_vertices_backward[s_i]]
                  .vertex = i;
          compressed_incoming_adj_vertices_backward[s_i] += 1;
          compressed_incoming_adj_index_backward
              [s_i][compressed_incoming_adj_vertices_backward[s_i]]
      ...
      for (VertexId p_v_i = 0;
           p_v_i < compressed_incoming_adj_vertices_backward[s_i]; p_v_i++) {
        VertexId v_i =
            compressed_incoming_adj_index_backward[s_i][p_v_i].vertex;
        incoming_adj_index_backward[s_i][v_i] =
            compressed_incoming_adj_index_backward[s_i][p_v_i].index;
        incoming_adj_index_backward[s_i][v_i + 1] =
            compressed_incoming_adj_index_backward[s_i][p_v_i + 1].index;
      }
      ...
      for (EdgeId e_i = 0; e_i < curr_read_edges; e_i++) {
        ...
        for (int s_i = 0; s_i < sockets; s_i++) {
          if (local_partition_offset[s_i] <= dst &&
              dst < local_partition_offset[s_i + 1]) {
            EdgeId pos =
                __sync_fetch_and_add(&incoming_adj_index_backward[s_i][src], 1);
            incoming_adj_list_backward[s_i][pos].neighbour = dst;
            if (!std::is_same<EdgeData, Empty>::value) {
              incoming_adj_list_backward[s_i][pos].edge_data =
                  read_edge_buffer[e_i].edge_data;
            }
          }
        }
      }
    ...
    for (int s_i = 0; s_i < sockets; s_i++) {
      for (VertexId p_v_i = 0;
           p_v_i < this->compressed_incoming_adj_vertices_backward[s_i];
           p_v_i++) {
        VertexId v_i =
            this->compressed_incoming_adj_index_backward[s_i][p_v_i].vertex;
        incoming_adj_index_backward[s_i][v_i] =
            compressed_incoming_adj_index_backward[s_i][p_v_i].index;
        incoming_adj_index_backward[s_i][v_i + 1] =
            compressed_incoming_adj_index_backward[s_i][p_v_i + 1].index;
  ...
  R process_edges_backward(
    ...
    for (int i = 0; i < partitions; i++) {
      for (int s_i = 0; s_i < sockets; s_i++) {
        recv_buffer[i][s_i]->resize(sizeofM<M>(feature_size) * owned_vertices *
                                    sockets);
